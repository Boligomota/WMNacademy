...
Las Rules son clave porque le dicen a Cursor cómo comportarse siempre dentro de este proyecto. Sirven para inyectar contexto reutilizable y pueden vivir como Project Rules dentro de `.cursor/rules`, normalmente en archivos `.mdc`.
...
Las Skills son el corazón del método. Cursor las usa como paquetes de instrucciones reutilizables que pueden incluir `SKILL.md`, referencias, scripts y assets. Vamos a crear 4 Skills.
...
Los Subagents sirven para dividir tareas complejas, mantener contexto aislado y trabajar en paralelo. Vamos a crear 3 Subagents.
...
MCP permite conectar Cursor con herramientas y fuentes externas mediante servidores que exponen tools, prompts o recursos. En los screenshots vimos varios MCPs activos: Chrome DevTools, context7, gsc-server, Mercado Pago, sequential-thinking, Supabase, entre otros.
...
Hooks permite correr scripts en puntos específicos de la ejecución del agente para modificar comportamiento, reforzar políticas o agregar logging.
